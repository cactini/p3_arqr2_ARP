'''
    ...
    2019 EPS-UAM
    Modificado por Pablo Nicholas McGettrick durante practica 2 AR2 2026
'''
...
def getHwAddr(interface: str):  # VIENE HECHO NO MODIFICAR
    '''
        Nombre: getHwAddr
...
def process_Ethernet_frame(us: ctypes.c_void_p, header: pcap_pkthdr, data: bytes) -> None:
    ...
    if (etherType in EthernetProtocols):
        payload = data[14:]  # payload es el mensaje sin incluir cabeceras
...
def startEthernetLevel(interface: str) -> int:
    ...
    errbuf = bytearray(256)  # la funcion pcap_open_live en rc1_pcap.py tiene un quinto parametro llamado errbuf que no sale en ningun lado aqui. he preferido meterlo para que no salte error

    handle = pcap_open_live(interface, ETH_FRAME_MAX, PROMISC, TO_MS, errbuf)  # encode es para eth0 o WLAN, etc en formado bytes, TO_MS es un timeout en ms
...
def stopEthernetLevel() -> int:
    ...
    if not levelInitialized:
        return -1  # tiene que haber acabado la funcion anterior para que vaya esta

    if recvThread is not None:
        recvThread.stop()  # esta clase ya viene hecha arriba, y nos dicen que NO la modifiquemos.
...
def sendEthernetFrame(data: bytes, length: int, etherType: int, dstMac: bytes) -> int:
    ...
    if (length + 14) > ETH_FRAME_MAX:  # 14 del tamaño de cabecera
        return -1
    etherType_bytes = struct.pack('!H', etherType)

    header = dstMac + macAddress + etherType_bytes  # Mac destino, mac origen, ethertype

    frame = header + data

    if (len(frame) < ETH_FRAME_MIN):
        faltan = ETH_FRAME_MIN - len(frame)  # con cuantos bytes hay que hacer padding
        frame = frame + (b'\x00' * faltan)  # cadena de bytes nulos

    bytes_enviados = pcap_inject(handle, frame, len(frame))  # tambien en rc1_pcap.py, es la ultima funcion
